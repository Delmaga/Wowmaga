...
def verify_state(state: str):
    try:
        did, sig = state.split(".", 1)
        exp = hmac.new(SECRET_KEY.encode(), did.encode(), hashlib.sha256).hexdigest()[:20]
...
async def save_user_link(discord_id: int, region: str, access_token: str, refresh_token: str, expires_in: int, battletag: str = None):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            ...
            "region=excluded.region, battletag=excluded.battletag, "
            "access_token=excluded.access_token, refresh_token=excluded.refresh_token, expires_at=excluded.expires_at",
            (discord_id, region, battletag, access_token, refresh_token, int(time.time()) + expires_in),
        )
        await db.commit()
